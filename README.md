...
**一句话定位**：Conclave 是一个开源的多 agent 共识协作基座——让「系统应该怎么表现」的每一个结论都带证据、可独立复核、可度量。

**30 秒电梯陈述**：验证环节（测试、截图比对、流水线）早已高度自动化，共识环节——人脑里的业务模型与代码现实对齐——却没有。Conclave 把共识物化为结构化对象：一条结论 = 一条带证据锚点的条目，写进仓库里的**共识快照文件夹**（`conclave/<req-id>/`），随 git 版本化，任何人 clone 下来就能复核。多个异构模型对决策点做**独立盲评投票**（不辩论），门禁按信号自动放行或升级，人只出现在四类位置：**兜底、补充事实与上下文、决策、纠偏**。群聊只是交互入口，不是档案。

---

## 目录

- [1. 名字：为什么叫 Conclave](#1-名字为什么叫-conclave)
- [2. 30 秒读懂 Conclave](#2-30-秒读懂-conclave)
- [3. 为什么存在](#3-为什么存在)
...
**Conclave** 的本义是「闭门密议」：枢机主教被锁进一间与外界隔绝的密室，在选出结果之前不能与外部通声气，各自独立投票，直到达成共识。这个意象精确对应本项目的两个最核心的设计：

- **盲评独立投票**：投票 agent 之间不共享中间推理、不辩论，各自独立探索后提交——名字里的「隔绝」正是独立性保证的隐喻；
- **共识就是目标本身**：密议的产出不是某个交付物，而是「一群人真的对齐了」。本项目同样如此——共识不是副产品，它是被物化、被度量、被复核的那个对象。

「隔绝」还有第二层对应：事件流永不进 LLM 上下文、协调 agent 只持有最新快照（防上下文腐化），这些都是同一族隔离设计。

### 候选对比

| 候选 | 语义贴合度 | 主要风险 | 判断 |
|---|---|---|---|
| **Conclave** | **最高**：独立盲评投票 + 隔绝 + 共识即目标，三层含义全部对应 | npm 公开包名可用性未确认；宗教/仪式色彩可能引起部分人联想 | **推荐**。包名不可用时用 scope 兜底：`@conclave-dev/cli` 之类的组织 scope，不影响项目名 |
...
**结论：Conclave。** 它是唯一一个把「独立投票」和「共识是目标」同时编码进名字的候选；包名风险是可兜底的工程问题，语义风险不可兜底。
...
**差异点一句话**：harness 类工具解决「单 agent 任务的校验闭环」，Conclave 解决「多 agent + 多人多角色在同一需求上的共识与流转」。
