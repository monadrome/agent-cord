...
- **并发冲突 = 条件写入 + 冲突转人工**（ADR-0020）：状态流转事件携带 `expected_status`（基于该条目当前最新状态）；reducer 检测到事件基于已过期状态（如重验通过到达时条目已被推翻）时**不静默择胜**，落 `ledger.conflict_detected` 事件并升级人工裁决——机器无权在两个矛盾意图之间择胜。
...
投票器、门禁等系统模块同样只提交事件：「写投票记录」的实现含义是提交 `vote.completed` 等事件，由 reducer 投影进账本；**事件落盘成功前不向调用方返回成功**（ADR-0020 的原子边界）。
...
2. **事件流完整性校验**：校验 `event_id` 全局唯一、`prev_event_hash` 前驱链完整、以及 `seq` 在**单写者血统内**连续（血统内无空洞 = 未丢事件；跨分支合并造成的血统交错不算违例，口径见 ADR-0020）——**丢事件必须可见**，这是审计系统的最低要求。
...
- **自定义 merge driver**：在 `.gitattributes` 中为 `cord/**/events.jsonl` 注册自定义合并驱动，按 `event_id` 去重，合并后全序按**因果链拓扑序为主、`(timestamp, event_id)` 兜底**（spec-kitty 的生产事故记录 + ADR-0010 注意点 11 的实测校准给出的路子；git 内建 union 不去重不排序，不能单独使用）；合并后由 `doctor` 按 §6.4 口径校验。
